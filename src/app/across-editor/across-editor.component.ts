...
export class AcrossEditorComponent implements OnInit {
  ...
  updateRow(i, j) {
    this.previousGroupLevel = [];
    this.currentGroupLevel = [];
    if (i > 1 && j>=1) {
      for (let d = j; d >= 0; d--) {
        if (this.buttonArray[d - 1] != 0) {
          this.previousNonZeroDigitIndex = d-1;
          break;
        }
      }
      for (let g = this.previousNonZeroDigitIndex; g >= 0; g--) {
        if (g - 1 >= 0 && this.buttonArray[g - 1] == 0) {
          this.previousGroupLevel.push('1');
        } else {
          break;
        }
      }
      for (let g = j; g >= 0; g--) {
        if (this.buttonArray[g - 1] == 0) {
          this.currentGroupLevel.push('1');
        } else {
          break;
        }
      }
      let s = i;
      let m = j;
      if ((m - 3) >= 0 && this.previousNonZeroDigitIndex >0) {
        while (s < this.fndlFormatedFilteredData.tab[0].table[0].rows.length && s >= 0) {
          if (this.fndlFormatedFilteredData.tab[0].table[0].rows[s].columns[m-3].rowSpan == 0) {
            ...
            for (let f = (this.previousNonZeroDigitIndex - this.previousGroupLevel.length); f <= this.previousNonZeroDigitIndex; f++) {
  ...
  addRow(i, j) {
    ...
    this.triggerIndex = this.getTriggerIndex(j);
    if (j <= 1) {
      ...
      this.fndlFormatedFilteredData.tab[0].table[0].rows.splice(this.getCurrentRowIndex(i,j) + 1, 0, rowData);
      this.addMappings(j, i);
    }
  }

  getTriggerIndex(j: number) {
    for (let i = j; i >= 0; i--) {
      if (this.buttonArray[i - 1] != 0) {
        this.triggerIndex = i;
        break;
      }
    }
